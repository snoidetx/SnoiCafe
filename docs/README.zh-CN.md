...
2. 在 SQL Editor 中依次运行 [初始数据库脚本](../supabase/migrations/202609220001_kitchen.sql) 、[访问口令更新脚本](../supabase/migrations/202609220002_unrestricted_credentials.sql) 和 [主厨资料更新脚本](../supabase/migrations/202609220003_chef_profiles.sql)。初始脚本只在空项目运行一次；若已经执行过，按顺序运行两个更新脚本即可。
...
使用相同主厨名称会回到同一资料，忽略大小写与前后空格；不同名称代表不同家庭主厨，共用厨房的主厨密码，仅有名称不能获得主厨权限。设置中的改名会同步该资料的所有设备；不能改成另一位主厨已使用的名称。历史点单保留当时的名称。“主厨设备会话”可以退出单个设备，同时保留家庭资料。食客会话与各自点单权限保持独立。
...
- **数据库设置未完成：**确认三个 SQL 脚本都已在所连接的项目执行成功。初始建表脚本只在空数据库运行一次。
...
- **主厨资料：**家人列表与人数统计按主厨名称计算，多台设备只显示一次。主厨名称用于区分家人，知道主厨密码的人可以用任一主厨名称加入。
