...
- Menu orders and freeform dish wishes share a wishlist. Chefs serve or cancel pending requests and can permanently delete pending or cancelled requests; customers cancel their own pending requests. History retains the original dish name, price, options, and requester.
...
Chefs can choose **Delete** on a pending item in Wishlist or a cancelled item in **History**, then confirm its name. This permanently removes that request and its history record without deleting its menu dish. **Cancel request** remains available when you want to keep a record in History. Customers cannot delete requests; completed requests stay protected. Existing installations must run [migration 007](supabase/migrations/202609230007_delete_cancelled_requests.sql) after 006, then redeploy. Applying the migration itself removes no data.
